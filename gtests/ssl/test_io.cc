...
#define UNIMPLEMENTED                                           \
  fprintf(stderr,                                               \
    "Call to unimplemented function %s\n", __FUNCTION__);       \
  PR_ASSERT(PR_FALSE);                                          \
...
static int32_t DummySeek(PRFileDesc *f, int32_t offset,
                                  PRSeekWhence how) {
  UNIMPLEMENTED;
  return -1;
}

static int64_t DummySeek64(PRFileDesc *f, int64_t offset,
                                    PRSeekWhence how) {
  UNIMPLEMENTED;
  return -1;
}
...
static int32_t DummyWritev(PRFileDesc *f, const PRIOVec *iov,
...
void Poller::Wait(Event event, DummyPrSocket* adapter,
...
void Poller::SetTimer(uint32_t timer_ms,
                          PollTarget* target,
                          PollCallback cb,
                          Timer** timer) {
...
bool Poller::Poll() {
  ...
  if (!timers_.empty()) {
    ...
    if (now >= first_timer->deadline_) {
      ...
      timeout = PR_MillisecondsToInterval(
          (first_timer->deadline_ - now)/1000);
    }
  }
