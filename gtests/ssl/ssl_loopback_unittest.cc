 ...
 public:
  virtual void Inspect(DummyPrSocket* adapter,
    ...
    while(parser.NextRecord(&ct, &buf)) {
      OnRecord(adapter, ct, buf->data(), buf->len());
    }
  }

  virtual void OnRecord(DummyPrSocket* adapter,
...
class TlsAgent : public PollTarget {
 public:
  ...
  void SetPeer(TlsAgent* peer) {
    adapter_->SetPeer(peer->adapter_);
  }

  void SetInspector(Inspector* inspector) {
    adapter_->SetInspector(inspector);
  }
  ...
  const char* state_str(State state) const {
    return states[state];
  }
  ...
  void Handshake() {
    ...
    if (rv == SECSuccess) {
      ...
      rv = SSL_GetCipherSuiteInfo(info_.cipherSuite,
                                  &csinfo_, sizeof(csinfo_));
      ASSERT_EQ(SECSuccess, rv);
 ...
 private:
  ...
  void SetState(State state) {
    ...
    LOG("Changing state from " << state_str(state_)
        << " to " << state_str(state));
    state_ = state;
  }
...
{
  EnableSomeECDHECiphers();
  TlsInspectorRecordHandshakeMessage *i1 = new
      TlsInspectorRecordHandshakeMessage(kTlsHandshakeServerKeyExchange);
  ...
  // Restart
  Reset();
  TlsInspectorRecordHandshakeMessage *i2 = new
    TlsInspectorRecordHandshakeMessage(kTlsHandshakeServerKeyExchange);
...
{
  EnableSomeECDHECiphers();
  SECStatus rv = SSL_OptionSet(server_->ssl_fd(),
                               SSL_REUSE_SERVER_ECDHE_KEY, PR_FALSE);
  ASSERT_EQ(SECSuccess, rv);
  TlsInspectorRecordHandshakeMessage *i1 = new
      TlsInspectorRecordHandshakeMessage(kTlsHandshakeServerKeyExchange);
  server_->SetInspector(i1);
  ...
  Reset();
  EnableSomeECDHECiphers();
  rv = SSL_OptionSet(server_->ssl_fd(),
                     SSL_REUSE_SERVER_ECDHE_KEY, PR_FALSE);
  ASSERT_EQ(SECSuccess, rv);
  TlsInspectorRecordHandshakeMessage *i2 = new
    TlsInspectorRecordHandshakeMessage(kTlsHandshakeServerKeyExchange);
